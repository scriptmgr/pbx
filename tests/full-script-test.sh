#!/bin/bash
# =============================================================================
# PBX Full Script Test Suite — canonical full/comprehensive test suite
# referenced by AI.md's "Full test suite" testing rule.
# Covers: system health, services, Asterisk core, a live AMI call test,
# FreePBX modules, DB integrity, web, fax, security/firewall, management
# scripts, backup (incl. rclone remote sync), email, filesystem/permissions,
# TLS, voicemail, conference/features, cron, and install idempotency.
...
check_svc() {
    ...
    for s in "$@"; do
        if systemctl list-units --type=service 2>/dev/null | grep -qE -- "^\s+${s}\.service"; then
            ...
            found=1
            case "$st" in
                active)     ok "SERVICE $label ($s): running"; return 0 ;;
                activating) warn "SERVICE $label ($s): activating"; return 0 ;;
                *)          fail "SERVICE $label ($s): $st"; return 1 ;;
            esac
        fi
    done
...
grep -q -- "Asterisk" <<< "$VER" && ok "Version: $VER" || fail "Asterisk CLI not responding"
...
if [ -n "$AMI_USER" ] && [ -n "$AMI_SECRET" ]; then
    AMI_LOGIN_PAYLOAD="Action: Login\r\nUsername: ${AMI_USER}\r\nSecret: ${AMI_SECRET}\r\n\r\nAction: Logoff\r\n\r\n"
    AMIRES=$(timeout 4 bash -c '
        exec 3<>/dev/tcp/127.0.0.1/5038
        printf "%b" "$1" >&3
        sleep 1
        cat <&3
    ' _ "$AMI_LOGIN_PAYLOAD" 2>/dev/null || true)
    grep -q -- "Response: Success" <<< "$AMIRES" && ok "AMI login: authenticated" || warn "AMI login: failed (user=$AMI_USER)"
else
...
if [ -n "$AMI_USER" ] && [ -n "$AMI_SECRET" ]; then
    ...
    ORIGOUT=$(timeout 10 bash -c '
        ...
        sleep 0.5
        cat <&3
    ' _ "$AMI_USER" "$AMI_SECRET" "$CALLID" 2>/dev/null || true)
    grep -qiE -- "Response: (Success|Error)" <<< "$ORIGOUT" \
...
grep -qiE -- "Reload Complete|message.*Reload" <<< "$RLOUT" \
...
HBODY=$(_curl_body)
grep -qiE -- '"status"' <<< "$HBODY"   && ok "/health: has 'status' key"    || warn "/health: missing 'status' key"
grep -qiE -- '"asterisk"' <<< "$HBODY" && ok "/health: has 'asterisk' key"  || warn "/health: missing 'asterisk' key"
grep -qiE -- '"mariadb"|"database"' <<< "$HBODY" && ok "/health: has DB key" || warn "/health: missing DB key"
grep -qiE -- '"hylafax"|"fax"' <<< "$HBODY" && ok "/health: has fax key"    || warn "/health: missing fax key"

# FreePBX admin login page content
ADMIN_BODY=$(_curl_code "${WEB_BASE_URL}/admin/config.php" >/dev/null; _curl_body)
grep -qiE -- "html|FreePBX|login|password" <<< "$ADMIN_BODY" \
...
if [ "$BEHIND_PROXY" = "yes" ] && [ -n "$PROXY_HTTP_PORT" ]; then
    ...
    [ -n "$CERT" ] && ok "TLS: SSL cert valid — $(grep -- notAfter <<< "$CERT")" || warn "TLS: no cert returned"
...
if command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active firewalld >/dev/null 2>&1; then
    ...
    grep -qiE -- "http" <<< "$SVCS" && ok "firewalld: HTTP/HTTPS allowed" || warn "firewalld: HTTP not in allowed services"
    PORTS=$(firewall-cmd --list-ports 2>/dev/null)
    grep -q -- "5060" <<< "$PORTS" && ok "firewalld: SIP 5060 allowed" || warn "firewalld: SIP 5060 not in ports"
...
PERM_ROOT=$(grep -oE -- "PermitRootLogin [a-z-]+" <<< "$SSHD_CONF" | tail -1 | awk '{print $2}')
...
[ -n "$OUT" ] \
    && grep -qiE -- "asterisk|running|PBX|service" <<< "$OUT" \
...
grep -qiE -- "webmin|port|status|URL" <<< "$OUT" \
...
grep -qiE -- "success|complete|backup|OK" <<< "$BACKUP_OUT" \
...
grep -qiE -- "clean|backup|retain|dry" <<< "$OUT" \
    && ok "pbx-cleanup: runs without error" || warn "pbx-cleanup: no output"

# Remote backup via rclone (INSTALL_REMOTE_BACKUP=yes installs it; absent is
# expected/acceptable on a default install — see install_rclone() in install.sh)
if command -v rclone >/dev/null 2>&1; then
    ok "rclone: installed ($(rclone version 2>/dev/null | head -1))"
else
    warn "rclone: not installed (expected unless INSTALL_REMOTE_BACKUP=yes)"
fi
...
ALL_CRON=$(
    ...
    find /etc/cron.d /etc/cron.daily /etc/cron.weekly -type f 2>/dev/null | xargs cat 2>/dev/null
    crontab -u asterisk -l 2>/dev/null
    crontab -u root -l 2>/dev/null
)
grep -qiE -- "backup|pbx-backup" <<< "$ALL_CRON"   && ok "Cron: backup job present"    || warn "Cron: no backup job"
grep -qiE -- "cleanup|pbx-cleanup" <<< "$ALL_CRON" && ok "Cron: cleanup job present"   || warn "Cron: no cleanup job"
grep -qiE -- "fwconsole|freepbx" <<< "$ALL_CRON"   && ok "Cron: FreePBX cron present" || warn "Cron: no FreePBX cron"
